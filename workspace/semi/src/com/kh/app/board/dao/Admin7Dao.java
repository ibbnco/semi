...
public class Admin7Dao {
	...
	public List<AdminVo> selectList(Connection conn , PageVo pageVo) throws Exception {
		...
		String sql ="SELECT * FROM ( SELECT ROWNUM AS RNUM , TEMP.* FROM ( SELECT MEMBER_NO , MEMBER_ID , MEMBER_STATUS , MEMBER_GRADE , ENROLL_DATE , MEMBER_NAME , MEMBER_NICK , MEMBER_TEL , MEMBER_EMAIL , MEMBER_POINT FROM PRODUCT ORDER BY MEMBER_NO DESC ) TEMP ) WHERE RNUM BETWEEN ? AND ?";
