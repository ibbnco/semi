...
public class DibsController extends HttpServlet{
	...
	private DibsService ds = new DibsService();
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (loginMember != null) {
			try {
				...
				//서비스 호출
				List<DibsVo> dibsMovie = ds.selectList(pageVo);
				
				//화면
				req.setAttribute("dibsMovie", dibsMovie);
				req.setAttribute("pageVo", pageVo);
				req.getRequestDispatcher("/WEB-INF/views/myPage/dibsMovies.jsp").forward(req, resp);
				
			} catch (Exception e) {
				System.out.println("[ERROR] 찜한 영화 조회 중 예외발생");
				e.printStackTrace();
